...
* Fazer o `git clone` dos repositórios `esp-idf`, `esp32-arduino-lib-builder` e `arduino-esp32` para uma pasta temporária (`/tmp/myLibBuilder-repos`), caso ainda não tenham sido clonados. O `arduino-esp32` é clonado diretamente em `esp32-arduino-lib-builder/components/arduino`, que é onde o `build.sh` já espera encontrá-lo — assim o `build.sh` compila exatamente o checkout preparado pelo `run.py`, em vez de clonar sua própria cópia por cima
