...
* Fazer o `git clone` dos repositórios `esp-idf`, `esp32-arduino-lib-builder` e `arduino-esp32` para uma pasta temporária (`/tmp/myLibBuilder-repos`), caso ainda não tenham sido clonados
...
O workflow `.github/workflows/push.yml` executa a seguinte automação:

### build-libs
* Rodar `run.py -t {target}` para cada um dos targets `esp32, esp32s2, esp32s3, esp32c2, esp32c3, esp32c6, esp32h2, esp32p4, esp32p4_es, esp32c5, esp32c61` em paralelo (matrix)
...
* Compactar `out/tools/esp32-arduino-libs` em `{branch}_esp32-arduino-libs-{yyyymmdd-hhmmss}.zip`
* Copiar o `package_esp32_index.template.json` gerado
* Enviar o `.zip` e o `package_esp32_index.template.json` para a release/tag `builds` (criada automaticamente caso ela nao exista)
